...
function Group() {
  ...
  function onSubmit(e) {
    ...
    //----------------------------------------------------------------------
    // Check for potentially harmful content using regular expressions
    const harmfulContentRegex =
      /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/i;

    // Check if any of the input fields contain harmful content
    if (
      harmfulContentRegex.test(groupName) ||
      harmfulContentRegex.test(groupLeaderName) ||
      harmfulContentRegex.test(groupLeaderId) ||
      harmfulContentRegex.test(memberTwoName) ||
      harmfulContentRegex.test(memberTwoId) ||
      harmfulContentRegex.test(memberThreeName) ||
      harmfulContentRegex.test(memberThreeId) ||
      harmfulContentRegex.test(memberFourName) ||
      harmfulContentRegex.test(memberFourId)
    ) {
      // Display an alert if harmful content is detected
      alert(
        "Potentially harmful content detected. Please remove any script tags or suspicious content."
      );
      return;
    }

    // If no harmful content is detected, proceed with form submission
    const newGroup = {
      groupName,
      groupLeaderName,
      groupLeaderId,
    ...
    axios
      .post("http://localhost:8000/api/group/post", newGroup)
      .then((res) => {
        alert("Group Created");
          ...
          <div className="app-content">
            <div className={groupStyles.header}>
              ...
              <div className={groupStyles.groups}>
                <a href="/viewGroups" className="btn btn-primary">
